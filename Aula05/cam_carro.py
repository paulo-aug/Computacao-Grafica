...
# CÂMERA
# =========================
cam_x, cam_y, cam_z = 8, 10, 20   # posição (eye)
cam_lx, cam_ly, cam_lz = 0, 0, 0  # ponto observado (center)
cam_ux, cam_uy, cam_uz = 0, 1, 0  # vetor up
...
def update_camera():
    ...
    global cam_lx, cam_ly, cam_lz

    speed = 0.3

    # Vetor direção (para onde a câmera olha)
    dir_x = cam_lx - cam_x
    dir_y = cam_ly - cam_y
    dir_z = cam_lz - cam_z
    ...
    if keys.get(glfw.KEY_A):
        ...
        cam_lx += right_x * speed
        cam_lz += right_z * speed
...
def draw_ground():
    size = 50  # tamanho do chão

    glColor3f(0.3, 0.7, 0.3)  # verde
...
def main():
    ...
    # Cria janela
    window = glfw.create_window(1920, 1080, "Camera 3D", None, None)
